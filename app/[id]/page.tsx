  ...
  return (
    <div className="grid grid-cols-10 gap-4 items-center w-full">
      <Tabs
        ...
        className="w-full justify-center items-center col-span-8 h-full pt-8 overflow-auto min-h-[calc(100vh-64px)]"
      >
        <TabsList className="">
        ...
        <TabsContent value="article" className="h-full">
          <ReactMarkdown className="w-full border border-gray-200 rounded-md p-8 shadow-sm mb-4 article">
            {article}
          </ReactMarkdown>
        </TabsContent>
      ...
      <div className="h-full bg-white border-l border-gray-100 space-y-4 p-4 col-span-2 pt-16">
        <ul className="text-gray-600 no-list-style space-y-2">
          {headings
            ?.slice(1) // Skip the first heading
            .map((heading) => (
              <li key={heading?.id} className="text-gray-600 no-list-style">
